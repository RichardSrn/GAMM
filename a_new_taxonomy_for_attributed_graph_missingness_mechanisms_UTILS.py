...
#         """
#         current_mean = p_missing.mean().item()
#         # Avoid division by zero in pathological cases
#         scaling = target_missing_rate / (current_mean + eps)
#         p_adj = torch.clamp(p_missing * scaling, max=1.0)
#         return p_adj
#
...
#
#         Returns:
#         - mask (torch.Tensor): A boolean tensor of shape (n x d) where True indicates the entry is observed
...
#             # Create a mask by drawing independent Bernoulli trials with probability (1 - missing_rate) of being observed.
#             prob_tensor = torch.full((n, d), 1 - missing_rate, device=data.x.device, dtype=torch.float32)
...
#
#             # The aggregator function is assumed to extract from 'data' all the relevant information,
#             # e.g., node features, observed indicators, neighborhood features/structural properties,
#             # and return a tensor of missing probabilities (one per feature entry).
...
#             # Now, generate missing indicators.
#             # We generate a uniform random tensor and declare an entry missing if the draw is less than p_missing_calib.
#             rand_tensor = torch.rand(n, d, device=data.x.device)
#             # In the mask: True means the entry is observed, so we mark it as observed when rand >= calibrated missing probability.
#             mask = (rand_tensor >= p_missing_calib)
#         else:
#             raise ValueError("Mechanism not recognized. Please choose one of 'MCAR', 'MAR', or 'MNAR'.")
#
#         # Optional: Check that the generated mask has an overall missing-level close to missing_rate.
        ...
        for i in tqdm(range(self.max_iter), desc="RR - max_iter", leave=False, disable=self.disable_tqdm):
            ...
            for l in range(d):
                ...
                for k in range(self.niter):
                    ...
                    #         # non_missing = (~mask[:, j].bool()).nonzero(as_tuple=False).view(-1)
                    #         # idx2 = np.random.choice(non_missing.cpu().numpy(), self.batchsize, replace=(self.batchsize > len(non_missing)))
                    #         # X2 = X_filled[non_missing][torch.from_numpy(idx2).to(X.device)]
                    #     else:
                    #         idx2 = np.random.choice(n, self.batchsize, replace=False)
                    #         X2 = X_filled[idx2]
                    #     loss = loss + self.sk(X1, X2)
...
#             break
#
#     # Added return for imputed features
#     return F_filled
def griot(model=None,
          p=None,
          lr=1e-2,
